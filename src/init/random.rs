...
mod tests {
    ...
    fn random_bisect_all_parts_present() {
        ...
        let g = path_graph(20);
        let p = RandomBisect.partition(&g, 2, 7).unwrap();
    ...
    fn random_bisect_k1_all_zero() {
        let g = path_graph(10);
        let p = RandomBisect.partition(&g, 1, 0).unwrap();
    ...
    fn random_bisect_deterministic() {
        let g = path_graph(10);
        let p1 = RandomBisect.partition(&g, 2, 42).unwrap();
        let p2 = RandomBisect.partition(&g, 2, 42).unwrap();
        assert_eq!(
