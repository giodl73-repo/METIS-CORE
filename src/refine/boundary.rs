...
mod tests {
    ...
    fn boundary_insert_and_remove() {
        ...
        b.remove(0);
        assert!(!b.contains(0), "after remove, vertex 0 must not be on boundary");
    }
    ...
    fn boundary_iter_yields_all_boundary_vertices() {
        ...
        assert_eq!(boundary_verts, vec![1, 2],
    ...
    fn boundary_is_empty_and_len() {
        let g = path_graph(4);
        let p_full = Partition { assignment: vec![0, 0, 1, 1], k: 2, tpwgts: None };
        let p_none = Partition { assignment: vec![0; 4], k: 1, tpwgts: None };
